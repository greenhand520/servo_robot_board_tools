...
fn render_config_query(f: &mut Frame, area: Rect, app: &App) {
    ...
    lines.push(Line::from(vec![
        Span::styled("📋 查询所有配置", Style::default().fg(Color::Blue).add_modifier(Modifier::BOLD)),
        Span::raw("  "),
        Span::styled("[Esc]", Style::default().fg(Color::DarkGray)),
        Span::raw(" 关闭"),
    ]));
...
fn render_config_editor(f: &mut Frame, area: Rect, app: &App) {
    ...
    for (i, (config_type, value)) in editor.configs.iter().enumerate() {
