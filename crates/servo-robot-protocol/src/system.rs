...
impl SystemInfo {
    pub fn from_bytes(data: &[u8]) -> Result<Self, FrameError> {
        if data.len() < 28 {
            return Err(FrameError::PayloadTooShort { expected: 28, got: data.len() });
        }
        let mut o = 0;
        ...
        let spi_error_count = u16::from_le_bytes([data[o], data[o+1]]); o += 2;
        let uart_error_count = u16::from_le_bytes([data[o], data[o+1]]); o += 2;
        let usb_error_count = u16::from_le_bytes([data[o], data[o+1]]); o += 2;
        ...
        Ok(SystemInfo { device_id, uid, imu_id, uptime_s, cpu_usage_percent, free_heap_kb, stack_watermark_min_kb, i2c_error_count, spi_error_count, uart_error_count, usb_error_count, frames_sent_total, pd_request_voltage, pd_request_current })
    }
